...
class TaskMetadata(object):
    ...
    def __post_init__(self):
        ...
        if self.cache and not self.cache_version:
            raise ValueError(
                "Caching is enabled ``cache=True`` but ``cache_version`` is not set."
            )
        if self.cache_serialize and not self.cache:
            raise ValueError(
                "Cache serialize is enabled ``cache_serialize=True`` but ``cache`` is not enabled."
            )
...
class Task(object):
    ...
    ) -> Optional[_task_model.Container]:
        """
    ...
    def sandbox_execute(
        ...
        ctx = (
            ctx.current_context()
            .with_execution_state(es.with_params(user_space_params=b.build()))
            .build()
        )
...
class PythonTask(TrackedInstance, Task, Generic[T]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if disable_deck is not None and enable_deck is not None:
            raise ValueError(
                "disable_deck and enable_deck cannot both be set at the same time"
            )
        ...
        if self._python_interface.docstring:
            if self.docs is None:
                ...
                if self._python_interface.docstring.short_description:
                    cast(Documentation, self._docs).short_description = (
                        self._python_interface.docstring.short_description
                    )
    ...
    def compile(
        self, ctx: FlyteContext, *args, **kwargs
    ...
    def _literal_map_to_python_input(
        ...
        return TypeEngine.literal_map_to_kwargs(
            ctx, literal_map, self.python_interface.inputs
        )
                ...
                if omt is not None:
                    ...
                    if om:
                        ...
                        if om.additional_items:
                            ...
                            logger.info(
                                f"Adding {om.additional_items} additional metadata items {metadata} for {k}"
                            )
                        if om.dynamic_partitions or om.time_partition:
                            a = art_id.ArtifactID(
                                partitions=idl_partitions_from_dict(
                                    om.dynamic_partitions
                                ),
    ...
    def _write_decks(self, native_inputs, native_outputs_as_map, ctx, new_user_params):
        if self._disable_deck is False:
            ...
            for k, v in native_inputs.items():
                input_deck.append(
                    TypeEngine.to_html(ctx, v, self.get_type_for_input_var(k, v))
                )

            output_deck = Deck(OUTPUT)
            for k, v in native_outputs_as_map.items():
                output_deck.append(
                    TypeEngine.to_html(ctx, v, self.get_type_for_output_var(k, v))
                )
    ...
    async def _async_execute(
        self, native_inputs, native_outputs, ctx, exec_ctx, new_user_params
    ):
        native_outputs = await native_outputs
        native_outputs = self.post_execute(new_user_params, native_outputs)
        literals_map, native_outputs_as_map = self._output_to_literal_map(
        ...
        * ``DynamicJobSpec`` is returned when a dynamic workflow is executed
        ...
        with FlyteContextManager.with_context(
            ctx.with_execution_state(
                cast(ExecutionState, ctx.execution_state).with_params(
            ...
            if inspect.iscoroutine(native_outputs):
                ...
                return self._async_execute(
                    native_inputs, native_outputs, ctx, exec_ctx, new_user_params
                )
            ...
            self._write_decks(
                native_inputs, native_outputs_as_map, ctx, new_user_params
            )
    ...
    def post_execute(
        self, user_params: Optional[ExecutionParameters], rval: Any
    ) -> Any:
        """
